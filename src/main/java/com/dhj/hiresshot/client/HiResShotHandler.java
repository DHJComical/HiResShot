...
public class HiResShotHandler {
    ...
    public void startCapture() {
        ...
        if (mode != CaptureMode.CPU_UPSCALE && (tW > maxTexSize || tH > maxTexSize)) {
            mc.player.sendMessage(new TextComponent("§cError: Size " + tW + "x" + tH + " > GPU Max " + maxTexSize), mc.player.getUUID());
            mc.player.sendMessage(new TextComponent("§eTip: Switch to 'CPU_UPSCALE' mode."), mc.player.getUUID());
            return;
        }
